...
const MEDAL_DEFS = [
  ...
  { id: 'first_record',   emoji: '📈', check: ({ recordBeaten }) => recordBeaten >= 1 },
  { id: 'records_10',     emoji: '📊', check: ({ recordBeaten }) => recordBeaten >= 10 },
  { id: 'steps_5000',     emoji: '👟', check: ({ maxSteps }) => maxSteps >= 5000 },
  { id: 'steps_10000',    emoji: '🚶', check: ({ maxSteps }) => maxSteps >= 10000 },
  { id: 'steps_goal_1',   emoji: '🎯', check: ({ stepsGoalReached }) => stepsGoalReached >= 1 },
  { id: 'steps_goal_7',   emoji: '🏃', check: ({ stepsGoalReached }) => stepsGoalReached >= 7 },
  { id: 'steps_goal_30',  emoji: '🌍', check: ({ stepsGoalReached }) => stepsGoalReached >= 30 },
  { id: 'first_meal',     emoji: '🥗', check: ({ meals }) => meals >= 1 },
  { id: 'meals_50',       emoji: '🍽️', check: ({ meals }) => meals >= 50 },
  { id: 'meals_100',      emoji: '👨‍🍳', check: ({ meals }) => meals >= 100 },
  { id: 'first_weight',   emoji: '⚖️', check: ({ weightEntries }) => weightEntries >= 1 },
  { id: 'weight_7',       emoji: '📅', check: ({ weightEntries }) => weightEntries >= 7 },
  { id: 'weight_30',      emoji: '📆', check: ({ weightEntries }) => weightEntries >= 30 },
  { id: 'weight_goal',    emoji: '🎉', check: ({ weightGoalReached }) => weightGoalReached },
]
...
function MedalUnlockModal({ medal, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-6" onClick={onClose}>
      ...
      <div
        className="relative bg-gray-900 border border-indigo-500/30 rounded-3xl p-8 flex flex-col items-center gap-4 w-full max-w-sm shadow-2xl"
      ...
      >
        ...
        <div className="text-center">
          <p className="text-indigo-400 text-xs font-medium uppercase tracking-widest mb-1">{t('medals.unlocked')}</p>
...
export default function Medals() {
  ...
  const MEDALS = MEDAL_DEFS.map(m => ({
    ...m,
    name: t(`medals.${m.id}`),
    description: t(`medals.${m.id}_desc`),
  }))
    ...
    <>
      ...
      <div className="bg-gray-900 rounded-2xl p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-white font-semibold">{t('medals.title')}</h2>
        ...
        <div className="mb-4">
          ...
          <p className="text-gray-600 text-xs mt-1">{Math.round((unlocked.length / MEDALS.length) * 100)}{t('medals.pct')}</p>
        </div>
